...
def get_overview_metrics(db: Session, days: int = 30) -> dict:
    try:
        ...
        return {
            ...
            "dau": 0, "wau": 0, "mau": 0, "total_predictions": 0, "total_volume": 0.0,
            "total_markets": 0, "open_markets": 0, "conversion_rate": 0.0,
            "users_with_predictions": 0, "avg_session_seconds": round(float(avg_session)),
            "returning_rate": 0.0, "realtime_active": realtime_active,
            "categories": [], "growth_chart": [], "prediction_chart": [],
        }
...
def get_sessions_list(db: Session, page: int = 1, page_size: int = 50) -> dict:
    try:
        ...
        db.rollback()
        # Retry without the users join
        try:
            offset = (page - 1) * page_size
            total = db.execute(text("SELECT COUNT(*) FROM pulse_user_sessions")).scalar() or 0
            rows = db.execute(text("""
                SELECT s.id, s.user_id, s.started_at, s.ended_at, s.duration_seconds,
                       s.device, s.browser, s.referrer, s.landing_page, s.exit_page,
                       s.made_prediction, s.country, s.utm_source, s.utm_campaign
                FROM pulse_user_sessions s
                ORDER BY s.started_at DESC
...
def get_retention_data(db: Session) -> dict:
    try:
        ...
        db.rollback()
        return {
            "day1_retention": 0.0, "day7_retention": 0.0, "day30_retention": 0.0,
            "churned_users": 0, "cohort_table": [],
        }
