"""
Metrics queries.
db  = Pulse DB (pulse_* tables: pulse_user_sessions, pulse_analytics_events, etc.)
sdb = Scenara DB (users, events, predictions, scenarios, accounts, etc.) — read-only
"""
...
def get_overview_metrics(db: Session, sdb: Session, days: int = 30) -> dict:
    since = datetime.utcnow() - timedelta(days=days)
    today = datetime.utcnow().date()
    week_ago = datetime.utcnow() - timedelta(days=7)
    month_ago = datetime.utcnow() - timedelta(days=30)

    # --- Pulse-only metrics (always available) ---
    ...
    # --- Scenara metrics ---
    try:
        total_users = sdb.execute(text("SELECT COUNT(*) FROM users")).scalar() or 0
        new_today   = sdb.execute(text("SELECT COUNT(*) FROM users WHERE created_at::date = :d"), {"d": today}).scalar() or 0
        new_week    = sdb.execute(text("SELECT COUNT(*) FROM users WHERE created_at >= :s"), {"s": week_ago}).scalar() or 0
        new_month   = sdb.execute(text("SELECT COUNT(*) FROM users WHERE created_at >= :s"), {"s": month_ago}).scalar() or 0

        dau = sdb.execute(text("SELECT COUNT(DISTINCT user_id) FROM predictions WHERE created_at::date = :d"), {"d": today}).scalar() or 0
        wau = sdb.execute(text("SELECT COUNT(DISTINCT user_id) FROM predictions WHERE created_at >= :s"), {"s": week_ago}).scalar() or 0
        mau = sdb.execute(text("SELECT COUNT(DISTINCT user_id) FROM predictions WHERE created_at >= :s"), {"s": month_ago}).scalar() or 0

        total_predictions   = sdb.execute(text("SELECT COUNT(*) FROM predictions")).scalar() or 0
        total_volume        = sdb.execute(text("SELECT COALESCE(SUM(simulated_amount), 0) FROM predictions")).scalar() or 0
        total_markets       = sdb.execute(text("SELECT COUNT(*) FROM events")).scalar() or 0
        open_markets        = sdb.execute(text("SELECT COUNT(*) FROM events WHERE status = 'open'")).scalar() or 0
        users_with_preds    = sdb.execute(text("SELECT COUNT(DISTINCT user_id) FROM predictions")).scalar() or 0
        conversion_rate     = round((users_with_preds / total_users * 100) if total_users > 0 else 0, 1)

        returning = sdb.execute(text("""
            SELECT COUNT(*) FROM (
        ...
        sdb.rollback()
        total_users = new_today = new_week = new_month = 0
        dau = wau = mau = 0
        total_predictions = total_volume = total_markets = open_markets = 0
        users_with_preds = conversion_rate = returning_rate = 0
        categories = growth = pred_chart = []
    ...
    return {
        ...
        "users_with_predictions": users_with_preds,
...
def get_users_list(
    ...
    try:
        ...
        if filter == "new":
            ...
            where_clauses.append("u.id IN (SELECT DISTINCT user_id FROM predictions WHERE created_at >= NOW() - INTERVAL '7 days')")
        elif filter == "dormant":
            where_clauses.append("""
                u.id NOT IN (SELECT DISTINCT user_id FROM predictions WHERE created_at >= NOW() - INTERVAL '30 days')
            ...
            where_clauses.append("u.id IN (SELECT user_id FROM predictions GROUP BY user_id HAVING COUNT(*) = 1)")
        elif filter == "active_today":
            where_clauses.append("u.id IN (SELECT DISTINCT user_id FROM predictions WHERE created_at::date = CURRENT_DATE)")
        elif filter == "high_value":
            where_clauses.append("u.id IN (SELECT user_id FROM predictions GROUP BY user_id HAVING COUNT(*) >= 10)")

        where = " AND ".join(where_clauses)
        offset = (page - 1) * page_size

        total = sdb.execute(text(f"SELECT COUNT(DISTINCT u.id) FROM users u WHERE {where}"), params).scalar() or 0

        rows = sdb.execute(text(f"""
            SELECT
                u.id, u.email, u.display_name, u.created_at,
                u.xp, 0 as level, u.current_streak, u.best_streak,
        ...
        for r in rows:
            ...
            users.append({
                ...
                "wins": wins, "losses": r.losses or 0,
...
def get_user_detail(db: Session, sdb: Session, user_id: int) -> dict:
    try:
        row = sdb.execute(text("""
            SELECT
                u.id, u.email, u.display_name, u.created_at,
                u.xp, 0 as level, u.current_streak, u.best_streak,
                COALESCE(a.balance, 0) as balance,
                ...
                COUNT(CASE WHEN p.pnl > 0 THEN 1 END) as wins,
                COUNT(CASE WHEN p.pnl IS NOT NULL AND p.pnl <= 0 THEN 1 END) as losses,
                COALESCE(SUM(p.simulated_amount), 0) as tokens_spent,
        ...
        recent_preds = sdb.execute(text("""
            SELECT p.id, p.simulated_amount as amount, p.entry_probability, p.pnl, p.created_at,
        ...
        brier = sdb.execute(text("""
            SELECT COALESCE(1 - AVG(POWER(
    ...
    return {
        ...
        "current_streak": row.current_streak, "best_streak": row.best_streak,
        "balance": float(row.balance),
        "total_predictions": total_preds,
        "total_pnl": round(float(row.total_pnl or 0), 2),
        "wins": wins, "losses": row.losses or 0,
        "win_rate": round((wins / total_preds * 100) if total_preds > 0 else 0, 1),
        ...
        "recent_predictions": [
            {
                ...
                "entry_probability": float(p.entry_probability),
                "pnl": float(p.pnl) if p.pnl is not None else None,
                "created_at": p.created_at.isoformat(),
                "event_title": p.event_title, "category": p.category,
...
def get_sessions_list(db: Session, sdb: Session, page: int = 1, page_size: int = 50) -> dict:
    offset = (page - 1) * page_size
    total = db.execute(text("SELECT COUNT(*) FROM pulse_user_sessions")).scalar() or 0

    # Fetch pulse sessions from the pulse DB
    rows = db.execute(text("""
        SELECT id, user_id, started_at, ended_at, duration_seconds,
               device, browser, referrer, landing_page, exit_page,
               made_prediction, country, utm_source, utm_campaign
        FROM pulse_user_sessions
        ORDER BY started_at DESC
        LIMIT :limit OFFSET :offset
    """), {"limit": page_size, "offset": offset}).fetchall()

    # Enrich with email from Scenara DB where available
    user_ids = [r.user_id for r in rows if r.user_id]
    email_map: dict = {}
    if user_ids:
        try:
            email_rows = sdb.execute(
                text("SELECT id, email FROM users WHERE id = ANY(:ids)"),
                {"ids": user_ids},
            ).fetchall()
            email_map = {r.id: r.email for r in email_rows}
        except _SCENARA_ERR:
            sdb.rollback()
...
def get_markets_list(
    ...
    try:
        ...
        total = sdb.execute(text(f"SELECT COUNT(*) FROM events e WHERE {where}"), params).scalar() or 0
        rows = sdb.execute(text(f"""
            SELECT
                e.id, e.title, e.category, e.status, e.created_at, e.closes_at,
                COUNT(DISTINCT p.user_id) as unique_bettors,
                COUNT(p.id) as total_predictions,
                COALESCE(SUM(p.simulated_amount), 0) as total_volume,
                COALESCE(SUM(CASE WHEN s.status = 'won' THEN p.simulated_amount END), 0) as yes_volume,
                COALESCE(SUM(CASE WHEN s.status = 'lost' THEN p.simulated_amount END), 0) as no_volume,
                COALESCE(AVG(p.entry_probability), 0) as avg_confidence
            FROM events e
        ...
        sdb.rollback()
        return {"total": 0, "page": page, "page_size": page_size, "markets": []}


def get_market_detail(sdb: Session, market_id: int) -> dict:
    try:
        event = sdb.execute(text("""
        ...
        scenarios = sdb.execute(text("""
            SELECT s.id, s.title, s.probability, s.status,
                   (s.status = 'won') as is_winner,
                   COUNT(p.id) as bet_count,
                   COALESCE(SUM(p.simulated_amount), 0) as volume
            FROM scenarios s
            LEFT JOIN predictions p ON p.scenario_id = s.id
        ...
        top_predictors = sdb.execute(text("""
            SELECT u.id, u.display_name, u.email,
                   COUNT(p.id) as pred_count,
                   SUM(p.simulated_amount) as total_staked,
                   COALESCE(SUM(p.pnl), 0) as total_pnl
            FROM predictions p
            JOIN scenarios s ON s.id = p.scenario_id
        ...
        return {
            ...
            "total_volume": total_volume, "total_predictions": total_bets,
            "scenarios": [
                {
...
def get_predictions_list(
    ...
    try:
        ...
        rows = sdb.execute(text(f"""
            SELECT p.id, p.user_id, p.simulated_amount as amount, p.entry_probability, p.payout_multiplier,
                   p.pnl, p.created_at,
                   u.display_name, u.email,
                   e.id as event_id, e.title as event_title, e.category,
                   s.title as scenario_title, (s.status = 'won') as is_winner
            FROM predictions p
            JOIN scenarios s ON s.id = p.scenario_id
            JOIN events e ON e.id = s.event_id
            JOIN users u ON u.id = p.user_id
            ...
            "predictions": [
                {
                    ...
                    "entry_probability": float(r.entry_probability),
                    ...
                    "beat_crowd": float(r.entry_probability) > 50 and bool(r.is_winner),
                }
                for r in rows
            ],
        }
        ...
        sdb.rollback()
        return {"total": 0, "page": page, "page_size": page_size, "predictions": []}
...
def get_retention_data(sdb: Session) -> dict:
    try:
        cohort_rows = sdb.execute(text("""
            WITH cohorts AS (
                SELECT id as user_id, DATE_TRUNC('week', created_at) as signup_week
                FROM users WHERE created_at >= NOW() - INTERVAL '12 weeks'
            ),
            activity AS (
                SELECT user_id, DATE_TRUNC('week', created_at) as activity_week
                FROM predictions
                GROUP BY user_id, DATE_TRUNC('week', created_at)
            ),
            cohort_sizes AS (
                SELECT signup_week, COUNT(*) as cohort_size FROM cohorts GROUP BY signup_week
            )
            SELECT
                c.signup_week, cs.cohort_size,
            ...
            LEFT JOIN activity a ON a.user_id = c.user_id AND a.activity_week >= c.signup_week
        ...
        for r in cohort_rows:
            ...
            cohorts[week_key]["weeks"][week_n] = round(
                (r.retained / r.cohort_size * 100) if r.cohort_size > 0 else 0, 1
            )

        d1_total = sdb.execute(text("SELECT COUNT(*) FROM users WHERE created_at >= NOW() - INTERVAL '60 days'")).scalar() or 1
        d1 = sdb.execute(text("""
            SELECT COUNT(DISTINCT p.user_id) FROM users u
        ...
        d7 = sdb.execute(text("""
            ...
            WHERE u.created_at >= NOW() - INTERVAL '60 days'
        """)).scalar() or 0
        d30_total = sdb.execute(text("SELECT COUNT(*) FROM users WHERE created_at >= NOW() - INTERVAL '90 days'")).scalar() or 1
        d30 = sdb.execute(text("""
            SELECT COUNT(DISTINCT p.user_id) FROM users u
            JOIN predictions p ON p.user_id = u.id
                AND p.created_at BETWEEN u.created_at AND u.created_at + INTERVAL '30 days'
            WHERE u.created_at >= NOW() - INTERVAL '90 days'
        """)).scalar() or 0
        churned = sdb.execute(text("""
            SELECT COUNT(DISTINCT user_id) FROM (
                SELECT user_id, MAX(created_at) as last_pred FROM predictions GROUP BY user_id
        ...
        sdb.rollback()
        return {"day1_retention": 0.0, "day7_retention": 0.0, "day30_retention": 0.0, "churned_users": 0, "cohort_table": []}
...
def get_funnel_data(db: Session, sdb: Session) -> dict:
    ...
    try:
        total_users = sdb.execute(text("SELECT COUNT(*) FROM users")).scalar() or 0
        made_prediction = sdb.execute(text("SELECT COUNT(DISTINCT user_id) FROM predictions")).scalar() or 0
        made_two = sdb.execute(text(
            "SELECT COUNT(*) FROM (SELECT user_id FROM predictions GROUP BY user_id HAVING COUNT(*) >= 2) s"
        )).scalar() or 0
        returned_next_day = sdb.execute(text("""
        ...
        sdb.rollback()
        total_users = made_prediction = made_two = returned_next_day = 0
...
def get_leaderboard(sdb: Session, limit: int = 50, sort_by: str = "pnl") -> dict:
    try:
        order_map = {
            "pnl": "total_pnl DESC",
        ...
        rows = sdb.execute(text(f"""
            SELECT
                u.id, u.display_name, u.email, u.xp, 0 as level,
                ...
                CASE WHEN COUNT(p.id) > 0 THEN
                    COUNT(CASE WHEN p.pnl > 0 THEN 1 END)::float / COUNT(p.id) * 100
...
def get_realtime_data(db: Session, sdb: Session) -> dict:
    ...
    feed = []
    recent_preds_count = 0
    recent_signups_count = 0

    try:
        recent_preds = sdb.execute(text("""
            SELECT p.id, u.display_name, u.email, p.simulated_amount as amount, p.entry_probability,
        ...
        for p in recent_preds:
            feed.append({
                ...
                "timestamp": p.created_at.isoformat(), "color": "violet",
            })
        for s in recent_signups:
            feed.append({
                "type": "signup", "icon": "user-plus",
                "text": f"New user joined: {s.display_name or s.email}",
                "subtext": "Welcome to Scenara",
                "timestamp": s.created_at.isoformat(), "color": "blue",
            })
        for r in recent_resolved:
            feed.append({
                "type": "resolved", "icon": "check-circle",
    ...
    return {
        ...
        "feed": feed[:30],
        "recent_predictions_count": recent_preds_count,
        "recent_signups_count": recent_signups_count,
    }


# ---------------------------------------------------------------------------
# Daily metrics series (pulse DB only)
# ---------------------------------------------------------------------------
